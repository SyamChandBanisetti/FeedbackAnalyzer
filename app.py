...
if not gemini_api_key:
    ...
    try:
        ...
        st.session_state['gemini_model'] = genai.GenerativeModel(
            model_name="gemini-2.0-flash", # Using the specified model name
            safety_settings={
...
def detect_emotions(text_series):
    ...
    for text in text_series.astype(str).fillna(""):
        ...
        if not found_emotion_in_response:
            pass # We only count specific emotions, not a "neutral" here.
...
def generate_co_occurrence_graph(text_series, min_cooccurrence=2):
    ...
    # IMP FIX: Ensure there are nodes AND edges to draw a meaningful graph.
    # If not, return None to prevent errors when trying to draw/save an empty figure.
    if not G.nodes() or not G.edges():
        plt.close('all') # Close any potential figures created implicitly by networkx or matplotlib
        return None, "No strong co-occurrences or relevant words found for the graph with current settings. Try lowering the 'Minimum co-occurrence' threshold or ensure your data contains meaningful multi-word phrases."

    # Proceed with figure creation and drawing only if a meaningful graph exists
    ...
    <body>
        <div class="container">
            ...
            <div class="section">
                ...
                {analyses_results.get('gemini_summary_overall', '<p>No overall Gemini summary generated for report.</p>')}
        ...
        if st.sidebar.button("Generate HTML Report", key="generate_html_report_btn"):
            with st.spinner("Generating comprehensive report..."):
                ...
                if analysis_selections['co_occurrence_network']:
                    # Note: We use the same min_cooccurrence slider value from the UI
                    min_cooccurrence_report = st.session_state.min_cooccurrence_slider 
                    co_occurrence_fig_report, co_occurrence_message_report = generate_co_occurrence_graph(combined_feedback_series, min_cooccurrence_report)
